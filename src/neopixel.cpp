...
NeopixelInterface::NeopixelInterface(State *st)
  : strip(neopixelCount), animator(1) {
  state = st;
  strip.Begin();
}
...
void NeopixelInterface::loop() {
  ...
  switch (ifState) {
    ...
    case InterfaceState::ERROR:
      ...
      break;
    case InterfaceState::DIFF:
      if (millis() - ifChange > 2000) {
        startAnimationBlinkAll(RgbColor(255, 255, 0));
      } else {
        startAnimationRotate(RgbColor(255, 255, 0));
      }
      break;
  }
...
void NeopixelInterface::updateInterfaceState() {
  ...
  if (localState == SpaceState::SUNKNOWN) {  //should not happen
  ...
  if (localState != remoteState) {
    if (ifState != InterfaceState::DIFF) {
      ifState = InterfaceState::DIFF;
      ifChange = millis();
    }
    return;
  }
